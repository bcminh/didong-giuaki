...
public class Mon: NSManagedObject {
    static let entityName = "Mon"
    ...
    static func getByName(ten name:String) -> Mon? {
        ...
        do{
            ...
            print("Cannot get MonAn by name, \(error), \(error.userInfo)")
        }
    ...
    // Insert
    static func insert(ten name:String, gia price:Float, mota description:String, loaiMon type:LoaiMon) -> Mon? {
        let mon = create() as! Mon
        
        mon.ten = name
        mon.moTa = description
        mon.gia = price
        mon.loaiMon = type
        ...
        do {
            ...
            print("Cannot insert MonAn, \(error), \(error.userInfo)");
        }
